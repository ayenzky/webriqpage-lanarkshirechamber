	...
	init: function(ajax) {
      ...
      $.ajax({
          ...
          for (var i = 0; i < events.length; i++) {
            $('.list').append('<div class="day-event" date-day="'+ events[i].day +'" date-month="' + events[i].month +'" date-year="'+ events[i].year +'" data-number="'+ i +'"><a href="javascript:void(0)" class="close fa fa-times"></a><h2 class="title">'+ events[i].title +'</h2><p>'+ events[i].description +'</p></div>');
          }
    ...
    var mon = 'Mon';
		...
		function displayEvent() {
			$('tbody.event-calendar td').on('click', function(e) {
				$('.day-event').slideUp(500);
				var monthEvent = $(this).attr('date-month');
				var dayEvent = $(this).text();
				$('.day-event[date-month="' + monthEvent + '"][date-day="' + dayEvent + '"]').slideDown(500);
			});
		};
